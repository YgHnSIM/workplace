...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* 본문 공통 스타일 (1, 2페이지 완전히 동일한 글자 크기!) */
    .body-text {
      color: #1A1A1A;
      font-size: 36px;
      font-weight: 600;
...
function buildPdf(options = {}) {
  ...
  const tempHtmlPath = path.join(projectRoot, 'temp_statement_a2_v2.html');
  const outputPdfPath = options.outputPath || path.join(projectRoot, '성명서_20260921_A2_v2.pdf');
  ...
  try {
    ...
    console.log(`Rendering 2-page A2 PDF (v2) with ${chromePath}...`);
