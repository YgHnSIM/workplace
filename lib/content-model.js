...
function validateSourceFragment(filePath, projectRoot, errors) {
  ...
  const label = relativePath(projectRoot, filePath);
  if (!html.trim()) {
    errors.push(`${label}: statement body is empty`);
    return;
  }
  const allowedTags = new Set(['section', 'h2', 'p', 'div', 'ol', 'ul', 'li', 'strong', 'em', 'br']);
  const allowedClasses = new Set([
    'intro-section', 'section-title', 'body-text', 'no-indent', 'demands',
    'closing-block', 'closing-highlight', 'closing-text',
  ]);
  ...
  const visit = (node, ancestors = []) => {
    if (node.nodeName === '#comment' || node.nodeName === '#documentType') {
      errors.push(`${label}: statement fragment contains an unsupported comment or doctype`);
    }
    if (!node.tagName) {
      (node.childNodes || []).forEach((child) => visit(child, ancestors));
      return;
    }
    if (!allowedTags.has(node.tagName)) errors.push(`${label}: statement fragment contains unsupported <${node.tagName}> markup`);
    (node.attrs || []).forEach((attribute) => {
      if (attribute.name !== 'class') errors.push(`${label}: statement fragment contains unsupported ${attribute.name} attribute`);
    });
    String(node.attrs?.find((attribute) => attribute.name === 'class')?.value || '')
      .split(/\s+/).filter(Boolean).forEach((className) => {
        if (!allowedClasses.has(className)) errors.push(`${label}: statement fragment contains unsupported class ${className}`);
      });
    const classNames = new Set(String(node.attrs?.find((attribute) => attribute.name === 'class')?.value || '').split(/\s+/));
    if (node.tagName === 'section') sections += 1;
    if (node.tagName === 'h2' && classNames.has('section-title')) sectionTitles += 1;
    if (node.tagName === 'p' && classNames.has('body-text')) bodyParagraphs += 1;
    if (node.tagName === 'div' && classNames.has('demands')) demands += 1;
    if (node.tagName === 'li' && ancestors.some((ancestor) => ancestor.tagName === 'div'
      && String(ancestor.attrs?.find((attribute) => attribute.name === 'class')?.value || '').split(/\s+/).includes('demands'))) demandItems += 1;
    if (node.tagName === 'div' && classNames.has('closing-block')) closingBlocks.push(node);
    (node.childNodes || []).forEach((child) => visit(child, [...ancestors, node]));
  };
  visit(document);
  if (sections < 1 || sectionTitles < 1 || bodyParagraphs < 1) {
    errors.push(`${label}: statement fragment must contain sections, section-title headings, and body-text paragraphs`);
  }
  if (demands > 1 || (demands === 1 && demandItems < 1)) {
    errors.push(`${label}: statement demands block structure is invalid`);
  }
  if (closingBlocks.length !== 1) {
    errors.push(`${label}: statement fragment must contain one closing-block`);
  } else {
    const rows = (closingBlocks[0].childNodes || []).filter((node) => node.tagName);
    const rowClasses = rows.map((node) => new Set(String(node.attrs?.find((attribute) => attribute.name === 'class')?.value || '').split(/\s+/)));
    const validRows = rows.every((node, index) => node.tagName === 'p'
      && (rowClasses[index].has('closing-highlight') || rowClasses[index].has('closing-text')));
    const textRows = rowClasses.filter((classes) => classes.has('closing-text'));
    if (rows.length < 2 || !validRows || textRows.length !== 1 || !rowClasses.at(-1)?.has('closing-text')) {
      errors.push(`${label}: closing-block must end with one closing-text row`);
    }
  }
  if (/<(?:script|iframe|style|link)\b/i.test(html)) {
    errors.push(`${label}: statement fragment contains unsupported embedded markup`);
  }
}
